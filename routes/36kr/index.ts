...
import { getWafTokenId, listUrl, ProcessItem, rootUrl } from './utils';
...
async function handler(ctx) {
    ...
    const currentUrl = `${listUrl}${Object.hasOwn(shortcuts, path) ? shortcuts[path] : path}`;
    ...
    if (!itemListMatch) {
        throw new Error('Failed to extract article list from 36kr response; the page structure may have changed or access is blocked.');
    }
